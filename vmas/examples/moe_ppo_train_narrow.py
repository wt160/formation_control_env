...
clutter_pretrained_weights = torch.load('best_imitation_model_clutter_noise_5.pth', map_location=device)
...
for epoch in range(num_epochs):
    ...
    # env.render()
    # input("1")
    step_counters = torch.zeros(env.num_envs, device=device)  # [num_envs]
    ...
    for step in range(steps_per_epoch):
        ...
        done_override = (~desired_mask) | max_steps_reached  # [batch_size] bool tensor
    ...
    if epoch % 10 == 0:
        ...
        if avg_reward > best_evaluation_reward:
            ...
            # **Save the model**
            torch.save(clutter_actor_model.state_dict(), 'best_ppo_model_7dim_simple.pth')
