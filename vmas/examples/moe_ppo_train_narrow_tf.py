...
class GATActorTransformer(nn.Module):
    def __init__(self, in_channels, hidden_channels, action_dim, num_agents, num_transformer_layers=2, transformer_heads=4, dropout=0.1):
        """
        Initializes the GATActor model with an integrated Transformer Encoder.
        ...
        """
        super(GATActorTransformer, self).__init__()
        ...
        # Transformer Encoder
        encoder_layer = TransformerEncoderLayer(d_model=hidden_channels * 16, nhead=transformer_heads, dropout=dropout)
        self.transformer_encoder = TransformerEncoder(encoder_layer, num_layers=num_transformer_layers)
        ...
        # Optional: Positional Encoding (if agents have positional information)
        # Uncomment the following lines if you want to add positional encodings
        # self.positional_encoding = PositionalEncoding(hidden_channels * 16, dropout)

    def forward(self, data):
        """
        Forward pass of the GATActor with Transformer Encoder.

        Args:
            data (Batch): A batch of graph data containing node features, edge indices, and edge attributes.

        Returns:
            action_mean (torch.Tensor): Mean of the action distribution.
            action_std (torch.Tensor): Standard deviation of the action distribution.
        """
        x, edge_index, edge_attr = data.x, data.edge_index, data.edge_attr

        # GAT layers with ReLU activations
        ...
        agent_embeddings = self.extract_agent_embeddings(x, data.batch, data.num_graphs)  # Shape: [batch_size * num_agents, hidden_channels * 8]

        # Repeat graph embedding for each agent
        graph_embedding_repeated = graph_embedding.repeat_interleave(self.num_agents, dim=0)  # Shape: [batch_size * num_agents, hidden_channels * 8]

        # Concatenate agent embeddings with graph embeddings
        combined = torch.cat([agent_embeddings, graph_embedding_repeated], dim=1)  # Shape: [batch_size * num_agents, hidden_channels * 16]

        # Optional: Add positional encoding if necessary
        # combined = self.positional_encoding(combined)

        # Prepare input for Transformer: [sequence_length, batch_size * num_agents, d_model]
        # Since Transformer expects input shape as [seq_len, batch, d_model], and here we have single time step
        # We can add a dummy sequence length of 1
        combined = combined.unsqueeze(0)  # Shape: [1, batch_size * num_agents, hidden_channels * 16]

        # Pass through Transformer Encoder
        transformer_out = self.transformer_encoder(combined)  # Shape: [1, batch_size * num_agents, hidden_channels * 16]

        # Remove the dummy sequence length
        transformer_out = transformer_out.squeeze(0)  # Shape: [batch_size * num_agents, hidden_channels * 16]

        # Actor head
        actor_hidden = self.fc1(transformer_out)  # Shape: [batch_size * num_agents, hidden_channels * 4]
        action_mean = self.fc2(actor_hidden)  # Shape: [batch_size * num_agents, action_dim]
        action_mean = action_mean.view(data.num_graphs, self.num_agents, -1)  # Shape: [batch_size, num_agents, action_dim]
        action_std = torch.exp(self.log_std).expand_as(action_mean)
        # Compute action_std
        # action_std = torch.exp(self.log_std).expand_as(action_mean)  # Shape: [batch_size, num_agents, action_dim]

        return action_mean, action_std

    def extract_agent_embeddings(self, x, batch, batch_size):
        """
        Extracts embeddings for each agent from the node embeddings.

        Args:
            x (torch.Tensor): Node embeddings. Shape: [num_nodes, hidden_channels * 8]
            batch (torch.Tensor): Batch assignment for each node. Shape: [num_nodes]
            batch_size (int): Number of graphs in the batch.

        Returns:
            agent_embeddings (torch.Tensor): Embeddings for each agent. Shape: [batch_size * num_agents, hidden_channels * 8]
        """
        agent_node_indices = []
        ...
        agent_node_indices = torch.cat(agent_node_indices, dim=0)  # Shape: [batch_size * num_agents]
        agent_embeddings = x[agent_node_indices]  # Shape: [batch_size * num_agents, hidden_channels * 8]
...
actor_model = GATActorTransformer(
...
pretrained_weights = torch.load('best_gnn_model_7dim_tf.pth', map_location=device)
...
for epoch in range(num_epochs):
    ...
    for _ in range(ppo_epochs):
        ...
        for start in range(0, num_samples, mini_batch_size):
            ...
            action_mean_mb, action_std_mb = actor_model(data_list_mb)
    ...
    if epoch % 10 == 0:
        ...
        if avg_reward > best_evaluation_reward:
            ...
            # **Save the model**
            torch.save(actor_model.state_dict(), 'best_ppo_model_7dim_simple_tf.pth')
