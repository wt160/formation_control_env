...
# 初始化网络
clutter_actor_model = LaserFC_Actor(laser_dim=laser_dim, num_agents=num_agents).to(device)
...
for epoch_restart in range(eval_epoch_restart_num):
    env = VMASWrapper(
        scenario_name="formation_control_teacher_graph_obs_cuda1_conv_nominal_output_vel",
        num_envs=1,
        device=device,
    ...
    for step in range(steps_per_epoch):
        ...
        with torch.no_grad():
            ...
            action_mean = clutter_actor_model(obs['laser'], obs['relative_pos'], obs['nominal_pos'])  # Now returns action_std
        ...
        # action_env = action_env.squeeze(dim=2)
        # action_env = action_env.unsqueeze(dim=0)
        ...
        # agent_0_obs = next_obs[agent_0_name]
    ...
    save_video("eva_video/ppo_evaluation_imitation_{}_{}_{}_{}".format(current_time, epoch_restart, train_env_type, noise_level), frame_list, fps=1 / 0.1)
