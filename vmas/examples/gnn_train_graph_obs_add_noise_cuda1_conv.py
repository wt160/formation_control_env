...
for data_index, data_point in enumerate(collected_data):
    ...
    for batch_idx in range(batch_dim):
        ...
        for _ in range(noise_total_num):
            ...
            # 添加噪声到相对位置（位置+朝向）
            pos_noise = torch.randn_like(current_relative) * torch.tensor([0.05, 0.05, 0.01])
...
for epoch in range(num_epochs):
    ...
    for data in tqdm(train_loader, desc=f'Epoch {epoch+1}/{num_epochs}'):
        ...
        # print("data:{}".format(data))
        laser, relative, target = data
        # print("relative shape:{}".format(relative.shape))
        predicted_positions = conv_actor_net(laser, relative)  # Shape: [batch_size, num_agents, 2]
        ...
        loss = criterion(predicted_positions, target)
        loss.backward()
        optimizer.step()
