...
class TransformerActorWithLaser(nn.Module):
    def __init__(self, feature_dim, pose_dim, hidden_channels, action_dim, num_agents, x_limit, y_limit, theta_limit,
        ...
        # --- Actor Head ---
        self.fc_out_mean = nn.Linear(self.d_model, action_dim)
        self.fc_out_std = nn.Linear(self.d_model, action_dim)
    ...
    def forward(self, combined_input_tensor):
        ...
        action_mean_raw = self.fc_out_mean(follower_embeddings_flat)
        action_std_raw_params = self.fc_out_std(follower_embeddings_flat)
    ...
    for epoch in range(num_epochs):
            ...
            for step_idx in range(steps_per_epoch):
                ...
                with torch.no_grad():
                    if args.has_laser == True:
                        ...
                        action_mean, action_std = clutter_actor_model(obs_feature_tensor)  # Now returns action_std
                    else:
                    ...
                    state_values_per_env_per_agent = critic_model(obs_feature_tensor) # Shape: [train_num_envs, num_agents][train_num_envs]
        ...
        for _ in range(ppo_epochs):
            ...
            for start_idx in range(0, num_total_transitions, mini_batch_size_graphs):
                ...
                # Critic forward pass
                new_state_values_mb = critic_model(obs_data_list_mb) # [mb_len]
