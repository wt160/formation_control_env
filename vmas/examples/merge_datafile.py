...
def merge_pickled_data_list(input_file_list, output_file='collected_data_merged.pkl'):
    """
    ...
    for idx, file in enumerate(input_file_list, 1):
        try:
            with open(file, 'rb') as f:
                data = pickle.load(f)
                if isinstance(data, list):
                    merged_data.extend(data)
                    print(f"File {idx}/{len(input_file_list)} '{file}' loaded with {len(data)} records.")
                else:
...
if __name__ == "__main__":
    ...
    # merge_pickled_data(
    #     input_pattern='env_label_*.pkl',  # Pattern to match input files
    #     output_file='env_label_merged.pkl'  # Output file name
    # )

    input_file_list = ["collected_data_narrow_7.pkl", "collected_data_door_0.pkl", "collected_data_clutter_7.pkl"]
    output_file = "except_tunnel_env_data.pkl"
    merge_pickled_data_list(
        input_file_list,  # Pattern to match input files
