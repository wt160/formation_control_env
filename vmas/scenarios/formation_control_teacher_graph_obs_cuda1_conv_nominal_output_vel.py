                ...
                if self.current_formation_type == "ren_shape":
                    ...
                    if self.obstacle_pattern == 1:
                        ...
                        batch_index=None,
                            ...
                            agent.set_vel(
                                    torch.stack([agent.action.u[:, 0], agent.action.u[:, 1]], dim=-1) ,
                                batch_index=None,
                            )
                            agent.set_ang_vel(
                                torch.stack([agent.action.u[:, 2]], dim=-1),
                                batch_index=None,
                            )
    ...
    def observation(self, agent: Agent):
        ...
        if current_agent_index == 0:
            ...
            for d in range(BATCH_DIM):
                # 获取领队状态
                leader_pos = self.leader_robot.state.pos[d]  # [x, y]
                ...
                if near_obstacles.numel() > 0:
                    ...
                    for idx, bin_idx in enumerate(bin_indices):
                        if distances[idx] < laser_scan[bin_idx]:
                            laser_scan[bin_idx] = distances[idx]
                ...
                for follower in self.world.agents[1:]:  # 排除领队自己
                    rel_pos = follower.state.pos[d] - leader_pos
                    rel_theta = follower.state.rot[d] - leader_rot
