...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        if self.working_mode == "imitation":
            ...
            self.agent_radius = kwargs.get("agent_radius", 0.34)
        ...
        # self.bitmap = self.create_bitmap_obstacle("../train_maps", world, batch_dim)
...
# Add random noise to the second dimension of direction_2
    ...
    def reset_world_at_with_init_direction(self, env_index, init_direction):
        ...
        for i, agent in enumerate(self.world.agents):
            ...
            agent.set_pos(
                torch.tensor([rotated_x, rotated_y], device=self.world.device),
                batch_index=env_index,
            )
    ...
    def get_leader_paths(self, max_trials=10):
        """
        ...
        for dim in range(self.batch_dim):
            ...
            for trial in range(max_trials):
                ...
                target_found = False
                for _ in range(100):  # Try 100 random points before giving up on this trial
                    if random.random() < 0.5:
                ...
                path = self._find_path_a_star(start_coord, target_coord, bitmap, safety_radius=(agent_radius / scale + 4.0))
        ...
        for i, world_agent in enumerate(self.world.agents):
            ...
            if agent == world_agent:
                ...
                if self.current_formation_type == "ren_shape":
                    ...
                    print("action u:{}".format(agent.action.u))
                    print("action u shape:{}".format(agent.action.u.shape))
                    if self.working_mode == "world_model":
                        agent.state.force = agent.action.u[:, :2] - agent.state.vel[:, :2]
                        agent.state.torque = agent.action.u[:, 2].unsqueeze(-1) - agent.state.ang_vel[:, :1]
                        print("agent vel:{}".format(agent.state.vel))
    ...
    def observation(self, agent: Agent):
        ...
        # print("leader agent lidar:{}".format(lidar_observation_tensor))    
                ...
                if env_index < len(self.batch_leader_paths) and self.batch_leader_paths[env_index]:
                    ...
                    for i in range(len(path) - 1):
                        ...
                        line = rendering.Line(
                            (x1, y1),
                            (x2, y2),
                            width=0.2,  # Thicker than agent connections
                        )
                        xform = rendering.Transform()
                        line.add_attr(xform)
                    ...
                    for point in path:
                        ...
                        circle = rendering.make_circle(radius=0.1)
                        xform = rendering.Transform()
