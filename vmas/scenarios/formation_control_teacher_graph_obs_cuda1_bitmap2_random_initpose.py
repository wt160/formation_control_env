...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.OA_MAX_SPEED_X = 1.0  # m/s
        self.OA_MAX_SPEED_Y = 0.3  # m/s
        self.OA_MAX_SPEED_YAW = 1.0 # rad/s
        self.leader_vel_x = torch.zeros(self.batch_dim, device=device)
        self.leader_vel_y = torch.zeros(self.batch_dim, device=device)
        self.leader_ang_vel = torch.zeros(self.batch_dim, device=device)
...
# Add random noise to the second dimension of direction_2
    ...
    def reset_world_at_with_init_direction(self, env_index, init_direction, path):
        """
        ...
        self.leader_agent.set_pos(
            torch.tensor([path[0][0], path[0][1]], device=self.device),
        ...
        self.formation_center_pos[env_index, 0] = path[0][0]
        self.formation_center_pos[env_index, 1] = path[0][1]
        ...
        for i, agent in enumerate(self.world.agents):
            ...
            rotated_x = nominal_x * cos_rot - nominal_y * sin_rot + path[0][0]
            rotated_y = nominal_x * sin_rot + nominal_y * cos_rot + path[0][1]
    ...
    def get_leader_paths(self, max_trials=1000):
            ...
            for trial in range(max_trials):
                ...
                for _ in range(1000):  # Try 100 random points before giving up on this trial
                    if self.env_type == "bitmap_tunnel":
                        start_x = int(bitmap_width - 10)
                        start_y = int(bitmap_height / 2.0)
                    else:
                        start_x = random.randint(40, bitmap_width - 40)
                        start_y = random.randint(40, bitmap_height - 40)
                    start_coord = (start_y, start_x)
                    
                    if self._is_valid_point(start_coord, bitmap) and self._is_safe_point(start_coord, bitmap, agent_radius / scale + start_inflation_radius):
                        start_found = True
                        break
        ...
        for dim in range(self.batch_dim):
            ...
            self.reset_world_at_with_init_direction(dim, init_direction, self.batch_leader_paths[dim])
    ...
    def compute_path_following_controls(self, agent):
        ...
        for env_idx in range(batch_size):
            ...
            if env_idx < len(self.batch_leader_paths) and self.batch_leader_paths[env_idx]:
                ...
                self.leader_vel_x[env_idx] = speed *random_factor* torch.cos(leader_rot + angle_diff * 0.5)
                self.leader_vel_y[env_idx] = speed *random_factor* torch.sin(leader_rot + angle_diff * 0.5)
                self.leader_ang_vel[env_idx] = angular_gain * angle_diff
                # vel_x[env_idx] = 0.01
                # vel_y[env_idx] = 0.0
    ...
    def process_action(self, agent: Agent):
        self.velocity_limit = 1.0 # Adjusted speed for smoother movement
                ...
                if self.current_formation_type == "ren_shape":
                    ...
                    if self.obstacle_pattern == 1:
                        ...
                        #     )
                            ...
                            self.FOLLOWER_LINEAR_VEL_BUFFER = 0.1  # m/s
                            self.FOLLOWER_ANGULAR_VEL_BUFFER = 0.2 # rad/s
                            # Longitudinal velocity (vx) limits
                            max_vx_limit = torch.min(
                                self.leader_vel_x + self.FOLLOWER_LINEAR_VEL_BUFFER, 
                                torch.full_like(self.leader_vel_x, self.OA_MAX_SPEED_X)
                            )
                            min_vx_limit = torch.max(
                                self.leader_vel_x - self.FOLLOWER_LINEAR_VEL_BUFFER, 
                                torch.full_like(self.leader_vel_x, -self.OA_MAX_SPEED_X)
                            )

                            # Lateral velocity (vy) limits
                            # Lateral speed is often symmetric, so we limit its magnitude.
                            max_vy_limit = torch.min(
                                torch.abs(self.leader_vel_y) + self.FOLLOWER_LINEAR_VEL_BUFFER,
                                torch.full_like(self.leader_vel_y, self.OA_MAX_SPEED_Y)
                            )
                            min_vy_limit = -max_vy_limit

                            # Angular velocity (w) limits
                            max_w_limit = torch.min(
                                self.leader_ang_vel + self.FOLLOWER_ANGULAR_VEL_BUFFER,
                                torch.full_like(self.leader_ang_vel, self.OA_MAX_SPEED_YAW)
                            )
                            min_w_limit = torch.max(
                                self.leader_ang_vel - self.FOLLOWER_ANGULAR_VEL_BUFFER,
                                torch.full_like(self.leader_ang_vel, -self.OA_MAX_SPEED_YAW)
                            )

                            # --- 3. Clamp the desired velocities using the new dynamic limits ---
                            # torch.clamp supports tensor-wise min and max limits.
                            # vx_clamped = torch.clamp(vx_desired_unclamped, min_vx_limit, max_vx_limit)
                            # vy_clamped = torch.clamp(vy_desired_unclamped, min_vy_limit, max_vy_limit)
                            # w_clamped = torch.clamp(w_desired_unclamped, min_w_limit, max_w_limit)
    ...
    def compute_group_sector_reward(self):
        ...
        self.SECTOR_HALF_ANGLE = np.deg2rad(55) # rad, e.g., +/- 60 degrees from leader's back
        ...
        self.CENTROID_HALF_ANGLE = np.deg2rad(45) # rad, angular tolerance for the centroid
