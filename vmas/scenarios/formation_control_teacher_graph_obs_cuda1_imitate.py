...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.max_connection_distance = 1.74  # Example distance threshold
                    ...
                    # Check if both position errors are smaller than the threshold for each agent in the batch
                        if self.env_type == "door" or self.env_type == "door_and_narrow":
                            ...
                            if i == 3 :
                                ...
                                self.formation_goals[i][:, 0] = torch.where(close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.35 + world_agent.state.pos[:, 0]*0.45 + self.world.agents[4].state.pos[:, 0]*0.2)
                                self.formation_goals[i][:, 1] = torch.where(close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.35 + world_agent.state.pos[:, 1]*0.45 + self.world.agents[4].state.pos[:, 1]*0.2)
                            elif i == 4:
                                self.formation_goals[i][:, 0] = torch.where(close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.35 + world_agent.state.pos[:, 0]*0.45 + self.world.agents[3].state.pos[:, 0]*0.2)
                                self.formation_goals[i][:, 1] = torch.where(close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.35 + world_agent.state.pos[:, 1]*0.45 + self.world.agents[3].state.pos[:, 1]*0.2)
                            else:
                                self.formation_goals[i][:, 0] = torch.where(close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.35 + world_agent.state.pos[:, 0]*0.6 + self.world.agents[0].state.pos[:, 0]*0.05)
                                self.formation_goals[i][:, 1] = torch.where(close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.35 + world_agent.state.pos[:, 1]*0.6 + self.world.agents[0].state.pos[:, 1]*0.05)
                        elif self.env_type == "clutter":
                            threshold = 0.4
                            ...
                            if i == 3 :
                                ...
                                self.formation_goals[i][:, 0] = torch.where(target_close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.4 + world_agent.state.pos[:, 0]*0.3 + self.world.agents[1].state.pos[:, 0]*0.3)
                                self.formation_goals[i][:, 1] = torch.where(target_close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.4 + world_agent.state.pos[:, 1]*0.3 + self.world.agents[1].state.pos[:, 1]*0.3)
                                ...
                                self.formation_goals[i][:, 0] = torch.where(target_close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.4 + world_agent.state.pos[:, 0]*0.3 + self.world.agents[2].state.pos[:, 0]*0.3)
                                self.formation_goals[i][:, 1] = torch.where(target_close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.4 + world_agent.state.pos[:, 1]*0.3 + self.world.agents[2].state.pos[:, 1]*0.3)
                            elif i == 2 :
                                ...
                                self.formation_goals[i][:, 0] = torch.where(target_close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.4 + world_agent.state.pos[:, 0]*0.3 + self.world.agents[0].state.pos[:, 0]*0.3)
                                self.formation_goals[i][:, 1] = torch.where(target_close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.4 + world_agent.state.pos[:, 1]*0.3 + self.world.agents[0].state.pos[:, 1]*0.3)
                            elif i == 1 :
                            ...
                            close_to_goal = (abs(position_error_x) < threshold) & (abs(position_error_y) < threshold)
                            self.formation_goals[i][:, 0] = torch.where(close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.35 + world_agent.state.pos[:, 0]*0.65 )
                            self.formation_goals[i][:, 1] = torch.where(close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.35 + world_agent.state.pos[:, 1]*0.65 )
