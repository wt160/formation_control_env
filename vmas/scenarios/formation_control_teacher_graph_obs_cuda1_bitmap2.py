...
# Add random noise to the second dimension of direction_2
    ...
    def compute_group_center_reward(self):
        ...
        pos_error_sq = torch.sum(pos_error_vec**2, dim=1)
        orient_error = self._normalize_angle(current_group_avg_orient - desired_virtual_orient_global)
        orient_error_sq = orient_error.squeeze(-1)**2

        position_penalty = self.GROUP_CENTER_POS_REWARD_SCALE * pos_error_sq
        orientation_penalty = self.GROUP_CENTER_ORIENT_REWARD_SCALE * orient_error_sq
        ...
        per_follower_out_of_rect_penalty = self.OUT_OF_RECTANGLE_PENALTY_SCALE * (dx_out_sq + dy_out_sq)
