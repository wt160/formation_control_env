...
class Scenario(BaseScenario):
    def make_world(self, batch_dim: int, device: torch.device, **kwargs):
        ...
        self.min_collision_distance = 0.4
        ...
        train_maps_path = os.path.join(script_dir, "..", "train_maps_0_clutter")
...
# Add random noise to the second dimension of direction_2
    ...
    def reset_world_at_with_init_direction(self, env_index, init_direction):
        ...
        self.leader_robot.set_rot(init_direction, batch_index=env_index)
    ...
    def compute_agent_velocity(self, agent, agent_index):
        ...
        lidar_readings = lidar_sensor.measure()  # Shape: [batch_size, num_beams] (e.g., [batch_size, 20])
        ...
        # print("lidar_observation_tensor shape:{}".format(lidar_observation_tensor[0].shape))
        # lidar_observation_tensor shape:torch.Size([batch_size, 20])
        # n_rays = lidar_observation_tensor[0].shape[1]
    ...
    def single_agent_collision_obstacle_rew(self, agent):
        current_agent_index = self.world.agents.index(agent)
        lidar_observation_tensor = [agent.sensors[0]._max_range - agent.sensors[0].measure()]
        agent.collision_obstacle_rew

    def single_agent_reward_graph_formation_maintained(self, agent, rotation_weight=0.1):
        current_agent_index = self.world.agents.index(agent)
        ...
        for batch_idx in range(batch_size):
            # Extract current positions and rotations for all agents in the current batch
            # Resulting tensors will be [num_agents, num_features]
            # current_batch_actual_agent_positions = torch.stack(
            #     [current_agent_positions_dict[i][batch_idx] for i in range(num_agents)]
            # )
            # current_batch_actual_agent_rotations = torch.stack(
            #     [current_agent_rotations_dict[i][batch_idx] for i in range(num_agents)]
            # )

            # # Extract target goal positions and rotations for the current batch from the specs list
            # # Target positions for assignment: [num_agents, 2] (x,y)
            # current_batch_target_goal_positions_for_assignment = torch.stack(
            #     [formation_goals_specs_list[i][batch_idx, :2] for i in range(num_agents)]
            # )
            # # Target rotations for reward calculation: [num_agents, 1] (assuming rotation is scalar)
            # current_batch_target_goal_rotations = torch.stack(
            #     [formation_goals_specs_list[i][batch_idx, 2:3] for i in range(num_agents)] # Use slicing to keep dim
            # )
            
            # # print(f"Debug batch {batch_idx}: current_batch_actual_agent_positions shape: {current_batch_actual_agent_positions.shape}")
            # # print(f"Debug batch {batch_idx}: current_batch_target_goal_positions_for_assignment shape: {current_batch_target_goal_positions_for_assignment.shape}")
            # # print(f"Debug batch {batch_idx}: current_batch_target_goal_rotations shape: {current_batch_target_goal_rotations.shape}")


            # # Calculate the cost matrix based on positional distances for assignment
            # cost_matrix = torch.cdist(current_batch_actual_agent_positions,
            #                         current_batch_target_goal_positions_for_assignment)

            # # Solve the assignment problem (maps agent original indices to goal original indices)
            # row_ind, col_ind = linear_sum_assignment(cost_matrix.cpu().detach().numpy())
            
            # try:
            #     assignment_idx_for_current_agent = row_ind.tolist().index(current_agent_index)
            #     assigned_goal_original_idx = col_ind[assignment_idx_for_current_agent]
            # except ValueError:
            #     print(f"Error: current_agent_index {current_agent_index} not found in assignment row_ind {row_ind}")
            #     agent.pos_rew[batch_idx] = -100.0 # Penalize heavily
            #     continue
    ...
    def observation(self, agent: Agent):
        ...
        lidar_observation_tensor = [agent.sensors[0]._max_range - agent.sensors[0].measure()]
        # print("agent {} lidar:{}".format(current_agent_index, lidar_observation_tensor))    
        ...
        for d in range(self.world.batch_dim):
            ...
            if current_agent_index == 0:
                ...
                rel_theta = agent.state.rot[d] - leader_rot
                ...
                relative_poses.append(torch.cat([rotated_pos, rel_theta.unsqueeze(0)]))
        # print("lidar shape:{}".format(lidar_observation_tensor[0].shape))
