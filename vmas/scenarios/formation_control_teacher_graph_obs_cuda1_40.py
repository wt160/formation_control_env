...
# Add random noise to the second dimension of direction_2
        ...
        if self.is_evaluation_mode == False:
            ...
            device=self.device)
                ...
                center_list = []
                center_list.append(add_noise(torch.tensor([-0.5, 1.8], dtype=torch.float32, device=self.device), center_noise_level))
                center_list.append(add_noise(torch.tensor([-0.5, -1.8], dtype=torch.float32, device=self.device), center_noise_level))
                center_list.append(add_noise(torch.tensor([3.2, 0], dtype=torch.float32, device=self.device), center_noise_level))
            ...
            device=self.device)
                ...
                self.route_point_list.append(add_noise(torch.tensor([0.3, 0], dtype=torch.float32, device=self.device), 0.2))
                if random.random() < 0.5:
                    self.route_point_list.append(add_noise(torch.tensor([3.2, -2], dtype=torch.float32, device=self.device), 0.3))
                else:
                    self.route_point_list.append(add_noise(torch.tensor([3.2, 2], dtype=torch.float32, device=self.device), 0.3))
                ...
                if self.current_formation_type == "ren_shape":
                    ...
                    # Check if both position errors are smaller than the threshold for each agent in the batch
                        if self.env_type == "door" or self.env_type == "door_and_narrow":
                            ...
                            close_to_goal = (abs(position_error_x) < threshold) & (abs(position_error_y) < threshold)
                            self.formation_goals[i][:, 0] = torch.where(close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.27 + world_agent.state.pos[:, 0]*0.65 + self.world.agents[0].state.pos[:, 0]*0.08)
                            self.formation_goals[i][:, 1] = torch.where(close_to_goal, self.formation_goals[i][:, 1], 
                                                self.formation_goals[i][:, 1]*0.27 + world_agent.state.pos[:, 1]*0.65 + self.world.agents[0].state.pos[:, 1]*0.08)
                        else:
                            close_to_goal = (abs(position_error_x) < threshold) & (abs(position_error_y) < threshold)
                            self.formation_goals[i][:, 0] = torch.where(close_to_goal, self.formation_goals[i][:, 0], 
                                                self.formation_goals[i][:, 0]*0.3 + world_agent.state.pos[:, 0]*0.65 + self.world.agents[0].state.pos[:, 0]*0.05)
