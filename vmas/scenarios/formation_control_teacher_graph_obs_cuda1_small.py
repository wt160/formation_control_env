...
class Scenario(BaseScenario):
    ...
    def create_polygon_with_center(self, center, num_vertices = 8, polygon_radius = 0.5,sphere_radius=0.1, 
        ...
        #     center = torch.tensor(
        #         [
        #             np.random.uniform(-world_semidim + 3, world_semidim - 2),
    ...
    def spawn_obstacles(self, obstacle_pattern, env_index):
        ...
        def create_small_fixed_obstacle(
            ...
            for attempt in range(max_attempts):
                center = torch.tensor(
                    [
                        np.random.uniform(-self.world_semidim_x + 17, self.world_semidim_x - 7),
                        np.random.uniform(-self.world_semidim_y + 4, self.world_semidim_y - 4),
                    ],
                    dtype=torch.float32,
                    device=device,
                )
        ...
        def create_line_segment_between_pos(start_pos, end_pos, radius=0.1):
            """
...
# Add random noise to the second dimension of direction_2
        ...
        if self.is_evaluation_mode == False:
            if obstacle_pattern == 2:
                ...
                if len(line_segments) == 0:
                    ...
                    for poly_idx in range(num_polygons):
                        positions, outer_existing_centers, _ = create_small_fixed_obstacle(
                            existing_centers=outer_existing_centers,
                            sphere_radius=0.1,
                            min_center_distance=2.0,
                            world_semidim=self.world_semidim,
            ...
            device=self.device)
                ...
                # center_list.append(torch.tensor([0.5, -1.5], dtype=torch.float32, device=self.device))
                # # center_list.append(torch.tensor([3.5, 1.5], dtype=torch.float32, device=self.device))
                # # center_list.append(torch.tensor([3.5, -1.5], dtype=torch.float32, device=self.device))
                # center_list.append(torch.tensor([2.3, 0], dtype=torch.float32, device=self.device))
                # # center_list.append(torch.tensor([2.0, -3], dtype=torch.float32, device=self.device))
                # center_list.append(torch.tensor([-1.5, 3.5], dtype=torch.float32, device=self.device))
                # center_list.append(torch.tensor([-1.5, -3.5], dtype=torch.float32, device=self.device))
        ...
        #         device=device,
            ...
            if self.env_type == "narrow":
                center_list = []
                center_list.append(torch.tensor([-2.5, 1.3], dtype=torch.float32, device=self.device))
                center_list.append(torch.tensor([-2.5, -1.3], dtype=torch.float32, device=self.device))
                # center_list.append(torch.tensor([0.5, 1.5], dtype=torch.float32, device=self.device))
                # center_list.append(torch.tensor([0.5, -1.5], dtype=torch.float32, device=self.device))
                # # center_list.append(torch.tensor([3.5, 1.5], dtype=torch.float32, device=self.device))
        ...
        #         device=device,
            ...
            device=self.device)
                ...
                if len(line_segments) == 0:
                    ...
                    num_polygons = np.random.randint(8, 10)
                    # num_polygons = 1
                    # num_polygons = 5
                    outer_existing_centers = []
                    polygon_dict = {}
        ...
        if is_first:
            ...
            if formation_movement == "random":
                ...
                for dim in range(self.world.batch_dim):
                    ...
                    route_points = self.route_point_list  # Assuming this is the list of route points
                    ...
                    if available_route_points:
                        ...
                        nearest_point = available_route_points[torch.argmin(torch.tensor(distances))]
                        
                        # Add the nearest point to the visited list
                        self.visited_route_points.append(nearest_point.tolist())

                        # Compute the direction to the nearest route point
                        direction_to_point = nearest_point - current_pos
                    ...
                    tentative_next_pos = current_pos + step_direction
    ...
    def compute_agent_velocity(self, agent, agent_index):
        ...
        for dim in range(batch_size):
            ...
            obstacles = obstacle_manager.get_near_obstacles(current_pos[dim].cpu().numpy(), 0.8)  # Adjust search radius as needed
    ...
    def reward(self, agent: Agent):
        ...
        if is_first:
            ...
            for i, a in enumerate(self.world.agents):
                ...
                collision_free_mask = closest_dist > 0.5
        ...
        agent.connection_rew = 20*agent.connection_rew
        agent.action_diff_rew = 0.4*agent.action_diff_rew
        agent.target_collision_rew = 10*agent.target_collision_rew
        return agent.agent_collision_rew + agent.connection_rew + agent.action_diff_rew + agent.target_collision_rew 
    ...
    def observation(self, agent: Agent):
        ...
        if current_agent_index == 0:
            ...
            for d in range(self.world.batch_dim):
                ...
                near_obstacles = obstacle_manager.get_near_obstacles(leader_pos.cpu(), self.max_obstacle_include_range)
                ...
                if len(near_obstacles) != 0:
                    ...
                    current_graph = self.single_graph_from_data(d, nominal_formation_tensor[d, :, :], near_obstacles_in_leader_frame)

                else:
                    # current_graph = self.single_graph_from_data(nominal_formation_tensor[d, :, :], None, transformed_prev_positions_batch)
                    current_graph = self.single_graph_from_data(d, nominal_formation_tensor[d, :, :], None)
