...
# class CustomRewardSum(Transform):
#     def __init__(self, in_keys, out_keys):
#         super().__init__(in_keys, out_keys)
#         self.episode_reward = None

#     def reset(self, tensordict):
#         self.episode_reward = torch.zeros_like(tensordict.get(self.in_keys[0]))
#         return tensordict

#     def _call(self, tensordict):
#         reward = tensordict.get(self.in_keys[0])
#         if self.episode_reward is None:
#             self.episode_reward = torch.zeros_like(reward)
#         self.episode_reward += reward
#         tensordict.set(self.out_keys[0], self.episode_reward)
#         return tensordict

# # Use CustomRewardSum while defining the environment
# env = TransformedEnv(
#     env,
#     CustomRewardSum(in_keys=[env.reward_key], out_keys=[("agents", "episode_reward")]),
# )
...
def train(cfg: "DictConfig"):  # noqa: F821
    ...
    # RND Networks
    rnd_target_net = RNDNetwork(env.observation_spec["agents", "observation"].shape[-1], hidden_dim).to(cfg.train.device)
    rnd_predictor_net = RNDNetwork(env.observation_spec["agents", "observation"].shape[-1], hidden_dim).to(cfg.train.device)
    rnd_optimizer = torch.optim.Adam(rnd_predictor_net.parameters(), lr=cfg.train.lr)
    ...
    for i, tensordict_data in enumerate(collector):
        ...
        # Copy reward to top level
        data_view.set(env.reward_key, data_view.get(("next", "agents", "reward")))

        # RND intrinsic rewards
        intrinsic_rewards = []
        for state in data_view.get(("agents", "observation")):
            target_feature = rnd_target_net(state)
            predict_feature = rnd_predictor_net(state)
            intrinsic_reward = F.mse_loss(predict_feature, target_feature, reduction='none').mean(dim=-1)
            intrinsic_rewards.append(intrinsic_reward)

        intrinsic_rewards = torch.stack(intrinsic_rewards)
        intrinsic_rewards = (intrinsic_rewards - intrinsic_rewards.mean()) / (intrinsic_rewards.std() + 1e-10)
        combined_rewards = data_view.get(env.reward_key) + intrinsic_rewards.unsqueeze(-1)

        # Set combined reward back to next
        data_view.set(("next", "agents", "reward"), combined_rewards)
        ...
        for _ in range(cfg.train.num_epochs):
            for _ in range(cfg.collector.frames_per_batch // cfg.train.minibatch_size):
                ...
                rnd_loss = F.mse_loss(rnd_predictor_net(subdata.get(("agents", "observation"))), rnd_target_net(subdata.get(("agents", "observation"))))
                rnd_loss.backward()
                rnd_optimizer.step()
